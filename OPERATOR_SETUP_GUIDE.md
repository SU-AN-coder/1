...
这份文档只回答一件事：`接下来你本人需要做什么`。  
当前项目已经有一版可运行的 MVP 编排层骨架，但默认仍然使用内存存储，GitHub webhook 也还没有接真实的 GitHub App 鉴权和数据库持久化。

你现在的工作不是继续写业务逻辑，而是把运行环境和外部依赖准备好。等这些配置完成后，再把 PostgreSQL、Redis 和 GitHub App 接进来就会很顺。
...
你现在可以先把下面这些变量整理出来，等下一步代码接入时直接填。
...
等你配置完成后，下一轮直接把下面信息告诉我，我就可以继续把真实基础设施接进去：
